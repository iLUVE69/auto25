...
PWM_BAUD_RATE = 9600        # Must match Serial.begin() in the PWM Arduino
...
# YOLO Model
YOLO_MODEL_PATH = 'E:/Autonomous_vehicle_project/lane/best.pt' # Update if needed
...
PWM_NEAR = 170
PWM_FAR = 200
PWM_THRESHOLD = 0.0005  # When the current coordinate is within this range of any target, use PWM_NEAR
current_pwm_value = None  # To track the last set PWM value
...
def set_pwm_value(value):
    """
    Sends a PWM value to the Arduino controlling the motor speed.
    :param value: PWM value to send (integer between 0 and 255).
    """
    try:
        with serial.Serial(PWM_COM_PORT, PWM_BAUD_RATE, timeout=2) as ser:
            time.sleep(2)  # Allow time for Arduino to reset
            command = f"{value}\n"
            ser.write(command.encode('utf-8'))
            print(f"PWM command sent: {command.strip()}")
            response = ser.readline().decode('utf-8', errors='ignore').strip()
            print(f"PWM Arduino response: {response}")
    except serial.SerialException as e:
        print(f"PWM serial error: {e}")
    except Exception as e:
        print(f"Unexpected PWM error: {e}")

def is_near_any_target(lat, lon, threshold=PWM_THRESHOLD):
    """
    Checks if the given latitude and longitude are near any target location based on the PWM threshold.
    :param lat: Current latitude
    :param lon: Current longitude
    :param threshold: Threshold distance to consider as "near"
    :return: True if near any target; False otherwise.
    """
